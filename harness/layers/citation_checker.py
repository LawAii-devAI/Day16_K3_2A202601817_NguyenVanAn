...
def _matches_line(text: str, body: str) -> bool:
    ...
    if text in lines:
        return True
    text_s = text.strip()
    return any(text_s == line.strip() for line in lines)
...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if doc and _matches_line(text, doc.body):
                continue

            # Nguồn bị trỏ sai: tìm trong các tài liệu đã được quan sát nguyên vẹn
            found_doc_id = None
            for candidate in ctx.corpus.docs:
                if candidate.body in observed_text and _matches_line(text, candidate.body):
        ...
        # Cập nhật citations
        valid_citations = sorted(list(set(c["doc_id"] for c in claims if isinstance(c, dict) and c.get("doc_id"))))
